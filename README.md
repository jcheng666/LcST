...
### Missing Data (Imputation) Datasets

For imputation experiments (datasets with `MISSING` suffix, e.g.
`PEMS03MISSING`), the missing-value masks are constructed following the
protocol in [imputation_benchmark](https://github.com/wtl52656/imputation_benchmark).
Use `--dataset PEMS03MISSING` and point `--data_path` to the corresponding
`.npz` file generated by that benchmark pipeline.
